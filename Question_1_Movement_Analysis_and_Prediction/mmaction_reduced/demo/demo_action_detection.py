...
def visualize(args, frames, data_samples, action_label):
    ...
    for d, f in track_iter_progress(list(zip(data_samples, frames))):
        ...
        #cv2.putText(vis_frame, "Top 5 Predictions: ", (10, 30), FONTFACE, FONTSCALE / 2,FONTCOLOR, THICKNESS, LINETYPE)
        for i in range(5):
            cv2.putText(vis_frame, action_label[i], (10,20 + 20 * i), FONTFACE, FONTSCALE / 2,FONTCOLOR, THICKNESS, LINETYPE)
...
def main():
    ...
    video_file = 'demo/Test_Videos/Demo_Fedrer_Tennis.mp4' 
    ...
    results_1 = [(labels[k[0]], k[1]) for k in top5_label]
    ...
    for result_1 in results_1:
       if i <= 4:
           action = str(i+1) + ". " + result_1[0] + ": " + str(np.round(result_1[1],decimals = 3))
           action_label_1.append(action)
           i = i + 1
       print(f'{result_1[0]}: ', result_1[1])
    ...
    print("Result Label: ",action_label_1)
    visualize(args, frames, pose_data_samples, action_label_1)
